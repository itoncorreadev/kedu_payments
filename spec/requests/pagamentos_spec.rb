...
RSpec.describe "Registro de Pagamento", type: :request do
  describe "POST /cobrancas/:id/pagamentos" do
    context "when valid" do
      ...
      let(:cobranca) { plano.cobrancas.create!(valor_cents: 500_00, data_vencimento: Date.tomorrow, metodo_pagamento: :boleto, status: :emitida) }
    ...
    context "when canceled" do
      ...
      let(:centro_de_custo) { create(:centro_de_custo) }
      let(:plano_de_pagamento) { create(:plano_de_pagamento, responsavel_financeiro: responsavel_financeiro, centro_de_custo: centro_de_custo) }
      let(:cobranca) { plano.cobrancas.create!(valor_cents: 100_00, data_vencimento: Date.tomorrow, metodo_pagamento: :pix, status: :cancelada) }
    ...
    context "when zero value" do
      let(:responsavel_financeiro) { create(:responsavel_financeiro) }
      let(:centro_de_custo) { create(:centro_de_custo) }
      let(:plano) { create(:plano_de_pagamento, responsavel_financeiro: responsavel_financeiro, centro_de_custo: centro_de_custo) }
      let(:cobranca) { plano.cobrancas.create!(valor_cents: 100_00, data_vencimento: Date.tomorrow, metodo_pagamento: :pix, status: :emitida) }
